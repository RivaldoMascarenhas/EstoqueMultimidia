...
export function AssetLabelPrinter({
  ...
  const handlePrint = () => {
    const printElement = document.getElementById("assets-printable-container");
    if (!printElement) {
      window.print();
      return;
    }
    ...
    const htmlContent = `
      ...
      <html lang="pt-BR">
        <head>
          <meta charset="UTF-8">
          <title>Etiquetas Patrimoniais - UniFAP TI</title>
          <style>
            @page {
              size: A4 portrait;
              margin: 10mm;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[96vw] max-w-4xl max-h-[90vh] flex flex-col p-4 sm:p-6 rounded-3xl bg-card border-border shadow-2xl overflow-hidden">
        ...
        {/* Container de Etiquetas Patrimoniais Isolado para Impressão */}
        <div className="overflow-y-auto overflow-x-hidden pr-1 flex-1">
          <div id="assets-printable-container" className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 my-2">
                ...
                >
                  <div className="label-info min-w-0 flex-1 space-y-1 pr-2">
                    <div className="label-brand flex items-center gap-1.5 text-[9px] font-black tracking-wider text-blue-900 uppercase truncate">
                      ...
                      <span className="truncate">PATRIMÔNIO • UNIFAP TI</span>
                    </div>

                    <div className="label-tag text-lg font-black font-mono tracking-tight text-slate-950 truncate">
                      #{asset.assetTag}
                    </div>

                    <div className="label-title text-xs font-bold text-slate-800 line-clamp-1" title={asset.itemName}>
                      {asset.itemName}
                    </div>

                    {asset.model && (
                      <div className="label-model text-[10px] font-semibold text-slate-600 truncate">
                        Mod: {asset.model}
                      </div>
                    )}

                    {asset.serialNumber && (
                      <div className="label-serial text-[9px] font-mono text-slate-500 truncate">
                        S/N: {asset.serialNumber}
                      </div>
                    )}

                    {asset.boxCode && (
                      <div className="label-box text-[9px] text-emerald-700 font-bold truncate">
                        Caixa: {asset.boxCode}
                      </div>
                    )}

                    <div className="label-footer text-[8px] text-slate-400 italic pt-0.5 truncate">
                      Escaneie para consultar histórico
                    </div>
                  </div>
