...
export async function POST(req: NextRequest) {
  try {
    const { session, error } = await requireSession([Role.ADMIN, Role.OPERADOR, Role.GESTOR], { req: req });
    if (error) return error;
    ...
    return NextResponse.json(
      { success: false, error: formatZodError(error, "Erro ao registrar empréstimo.") },
      { status: 400 }
    );
  }
}
